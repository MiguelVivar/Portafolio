...
const CategoryFilter: React.FC<CategoryFilterProps> = ({ 
  ...
  return (
    ...
    >
      <div className="flex flex-wrap justify-center gap-3 p-3 bg-neutral-800/80 backdrop-blur-sm rounded-xl shadow-lg" role="group">
        {categorias.map((categoria) => (
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}            aria-pressed={categoriaActiva === categoria.id ? "true" : "false"}
            aria-label={`Filtrar por ${categoria.nombre}${conteo && conteo[categoria.id] !== undefined ? `, ${conteo[categoria.id]} proyectos` : ''}`}
          >
            <span className="text-current" aria-hidden="true">{categoria.icono}</span>
